...
import { useFarms, usePriceBnbBusd, usePools, usePriceAlbaBusd } from 'state/hooks'
...
const Pools: React.FC = () => {
  ...
  const cakebusd = usePriceAlbaBusd() // slimePriceInBusd
    ...
    React.useEffect(() => {
      async function fetchdata(_token, _isLp, _tokenDecimals,_slimeamm) {
        if (_token !== '') {
          ...
          if (mytoken !== '0xAb57000f39107eDE71441D54Aa53A22bdCE9F4B9') {
          ...
          } else if (mytoken === '0xAb57000f39107eDE71441D54Aa53A22bdCE9F4B9') {
  ...
  const StakeTokenPriceBNB = (token: string, tokenDecimals: number, isLpReward: boolean,isSlimeAMM:boolean): BigNumber => {
    ...
    React.useEffect(() => {
      async function fetchdata(_token, _isLp, _tokenDecimals,_slimeamm) {
        if (_token !== '') {
          // console.log('token STAKEE', _token)
          const mytoken = `0x${_token.substring(2).toUpperCase()}`
